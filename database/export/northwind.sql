...
--
-- Stand-in structuur voor view `argentinie`
-- (Zie onder voor de actuele view)
--
DROP VIEW IF EXISTS `argentinie`;
CREATE TABLE IF NOT EXISTS `argentinie` (
`ShipCountry` varchar(15)
,`ProductID` int(10) unsigned
,`SUM(order_details.Quantity)` decimal(27,0)
);
...
('MAISD', 'Maison Dewey', 'Catherine Dewey', 'Sales Agent', 'Rue Joseph-Bens 532', 'Bruxelles', '', 'B-1180', 'Netherlands', '(02) 201 24 67', '(02) 201 24 68'),
...
('SIMOB', 'Simons bistro', 'Jytte Petersen', 'Owner', 'Vinbæltet 34', 'København', '', '1734', 'Netherlands', '31 12 34 56', '31 13 35 57'),
...
('SUPRD', 'Suprêmes délices', 'Pascale Cartrain', 'Accounting Manager', 'Boulevard Tirou, 255', 'Charleroi', '', 'B-6000', 'Netherlands', '(071) 23 67 22 20', '(071) 23 67 22 21'),
...
('VAFFE', 'Vaffeljernet', 'Palle Ibsen', 'Sales Manager', 'Smagsløget 45', 'Århus', '', '8200', 'Netherlands', '86 21 32 43', '86 22 33 44'),
...
--
-- Stand-in structuur voor view `omzet_top3`
-- (Zie onder voor de actuele view)
--
DROP VIEW IF EXISTS `omzet_top3`;
CREATE TABLE IF NOT EXISTS `omzet_top3` (
...
(1, 'Chai', 1, 1, '10 boxes x 20 bags', 21.78, 39, 0, 10, 'n'),
(2, 'Chang', 1, 1, '24 - 12 oz bottles', 22.990000000000006, 17, 40, 25, 'n'),
(3, 'Aniseed Syrup', 1, 2, '12 - 550 ml bottles', 12.100000000000001, 13, 70, 25, 'n'),
...
--
-- Structuur voor de view `argentinie`
--
DROP TABLE IF EXISTS `argentinie`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `argentinie`  AS  select `orders`.`ShipCountry` AS `ShipCountry`,`order_details`.`ProductID` AS `ProductID`,sum(`order_details`.`Quantity`) AS `SUM(order_details.Quantity)` from (`orders` join `order_details` on((`orders`.`OrderID` = `order_details`.`OrderID`))) group by `order_details`.`ProductID`,`orders`.`ShipCountry` having (`orders`.`ShipCountry` = 'argentina') ;

-- --------------------------------------------------------

--
-- Structuur voor de view `omzet_top3`
--
DROP TABLE IF EXISTS `omzet_top3`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `omzet_top3`  AS  select `orders`.`EmployeeID` AS `EmployeeID`,sum((`order_details`.`Quantity` * `order_details`.`UnitPrice`)) AS `Omzet` from (`order_details` join `orders` on((`orders`.`OrderID` = `order_details`.`OrderID`))) group by `orders`.`EmployeeID` order by `Omzet` desc limit 3 ;
